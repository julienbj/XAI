...
plot_times <- function(m3, m4, m5, rho_nr=3, blue=F, filename=""){
  if (blue) {
  ...
  combined <- sort_data(m3, m4, m5, rho_nr = rho_nr)
  ...
  print(ggplot(summary_data, aes(x = Features, y = mean, color = Type, fill = Type)) +
    ...
    theme_minimal() +
    labs(title = "Method Efficiency Across Feature Count", y = "Time (s)", x = "Features") +
    theme(legend.title = element_blank(), plot.title = element_text(hjust = 0.35, size = 16),
...
sort_data <- function(m3, m4, m5, rho_nr){
  m3 <- lapply(m3, `[[`, "times")[[rho_nr]]
  m4 <- lapply(m4, `[[`, "times")[[rho_nr]]
  m5 <- lapply(m5, `[[`, "times")[[rho_nr]]
  
  make_df <- function(method_name, f3, f4, f5) {
    data.frame(
      Features = rep(c(3, 4, 5), times = c(length(f3), length(f4), length(f5))),
      Times = c(f3, f4, f5),
      Type = method_name
    )
  }
  ...
  combined <- bind_rows(
    make_df("copula", m3$copula, m4$copula, m5$copula),
    make_df("empirical", m3$empirical, m4$empirical, m5$empirical),
    make_df("gaussian", m3$gaussian, m4$gaussian, m5$gaussian),
    make_df("independence", m3$independence, m4$independence, m5$independence)
  )
